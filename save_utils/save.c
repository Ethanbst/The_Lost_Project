...
void save_progress(char* world_name, int battles_done[], int dialogs_done[]) {
    ...
    //Ajout des batailles effectuées 
    cJSON *dialogs = cJSON_CreateIntArray(dialogs_done, NB_DIALOGS);
    cJSON_AddItemToObject(root, "dialogs_done", dialogs);
...
//Fonction qui retourne le tableau de dialogue 1 si déjà déclanché, 0 sinon
int* get_dialogs(){
    FILE *file = fopen("save.json", "r");
    if (file == NULL) {
        add_log_error("save.c - get_dialogs()", "Erreur d'ouverture du fichier save.json");
        exit(1);
    }
    ...
    char *json_string = (char *)malloc(fsize + 1);
    if (json_string == NULL) {
        add_log_error("save.c - get_dialogs()", "Erreur d'allocation de la chaine JSON");
        exit(1);
    }
    ...
    cJSON *root = cJSON_Parse(json_string);
    if (root == NULL) {
        add_log_error("save.c - get_dialogs()", "Erreur de parsing du fichier save.json");
        exit(1);
    }

    cJSON *dialogs = cJSON_GetObjectItem(root, "dialogs_done");
    if (dialogs == NULL) {
        add_log_error("save.c - get_dialogs()", "Erreur de récupération des batailles");
        exit(1);
    }

    int* dialogs_done = (int *)malloc(NB_DIALOGS * sizeof(int));

    for (int i = 0; i < NB_DIALOGS; i++) {
        dialogs_done[i] = cJSON_GetArrayItem(dialogs, i)->valueint;
    }
